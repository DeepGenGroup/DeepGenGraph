...
class Attention(nn.Module):
  ...
  def forward(self, x, kv_caches):
    ...
    q_len = q.shape[1]
    kv_len = k.shape[1]
    std = torch.sqrt(roco_sq_score / q_len - (roco_score / q_len) ** 2)
    ...
    _, selected_indices_in_other = torch.topk(roco_score.gather(dim=-1, index=other_indices) / q_len, dim=-1, k=self.cache_budget)
    ...
    selected_indices = selected_indices.transpose(1, 2)[:, :, :, None].expand(q.shape[0], self.cache_budget + self.roco_recent, self.kv_head_num, self.head_dim)
