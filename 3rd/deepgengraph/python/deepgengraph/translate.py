...
def deepgengraph_from_onnx(onnx_model, func_name):
  context = ir.context()
  ir.deepgengraph.load_dialects(context)
  ...
  for node in graph.node:
    if node.op_type == "Reshape":
      ...
      out = builder.create_reduce(val_map[operand], axis, ir.deepgengraph.REDUCE_ADD, keep_dim == 1)
      assert len(node.output) == 1
      val_map[node.output[0]] = out
      ...
      lhs_val = find_val_or_create_tensor_const(lhs)
      rhs_val = find_val_or_create_tensor_const(rhs)
      out = builder.create_cmp(lhs_val, rhs_val, ir.deepgengraph.CMP_GE)
      assert len(node.output) == 1
      val_map[node.output[0]] = out
      ...
      lhs_val = find_val_or_create_tensor_const(lhs)
      rhs_val = find_val_or_create_tensor_const(rhs)
      out = builder.create_cmp(lhs_val, rhs_val, ir.deepgengraph.CMP_GT)
      assert len(node.output) == 1
      val_map[node.output[0]] = out
    else:
...
def onnx_from_deepgengraph_kernel(module, kernel_name, simplify=True):
  ...
  def walk_fn(op):
    ...
    if name == "arith.constant":
    ...
    elif name == "deepgengraph.reshape":
      ...
      nodes.append(node)
      val_map[op.get_result(0)] = res_name
    elif name == "deepgengraph.permute":
      dims = ir.deepgengraph.permute.from_operation(op).get_dims()
      ...
      nodes.append(node)
      val_map[op.get_result(0)] = res_name
    elif name == "deepgengraph.convert":
      arg = op.get_operand(0)
      dst_type = ir.deepgengraph.convert.from_operation(op).get_dst_type()
      ...
      nodes.append(node)
      val_map[op.get_result(0)] = res_name
    elif name == "deepgengraph.reduce":
      arg = op.get_operand(0)
      res = op.get_result(0)
      res_name = f"reduce_{counter.inc()}"
      op = ir.deepgengraph.reduce.from_operation(op)
    ...
    elif name == "deepgengraph.softmax":
      arg = op.get_operand(0)
      res = op.get_result(0)
      res_name = f"softmax_{counter.inc()}"
      op = ir.deepgengraph.softmax.from_operation(op)
      dim = op.get_reduce_dim()
      ...
      op = ir.deepgengraph.trilu.from_operation(op)
...
if __name__ == "__main__":
  import onnx
  fn = "/home/zhongrx/llm/tune/deepgengraph/cases/AttnH2O.onnx"
  onnx_model = onnx.load_model(fn)
  # print(f"{onnx_model}")
  module = deepgengraph_from_onnx(onnx_model, "attn_h2o")
  print(f"{module=}")
