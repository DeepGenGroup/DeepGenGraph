...
extractThreadTileFromVector(Value fullVector, VectorType tileTy, LowerInfo &info,
...
public:
  ...
  LogicalResult matchAndRewrite(GemmOp op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const override {
    ...
    if(s_hw->getKind() == HW_KIND_DCU){
      ...
      auto getVectorBackedValue = [&](Value original, Value adapted) -> Value {
      ...
      auto materializeThreadTile = [&](Value vector, VectorType vecTy,
        ...
        auto tile = extractThreadTileFromVector(vector, vecTy, info, rewriter,
      ...
      if (Value aVector = getVectorBackedValue(op.getA(), adaptor.getA())) {
        auto materializedA =
      ...
      if (Value bVector = getVectorBackedValue(op.getB(), adaptor.getB())) {
        auto materializedB =
...
public:
  ...
  LogicalResult matchAndRewrite(frisk::MaskOp op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const override {
    ...
    s_buffer_replace[op.getResult()] = resultVector;
...
static VectorType getVecTypeByLowerInfoWithThreadOwnData(const LowerInfo& info){
  ...
  auto memTy = mlir::cast<MemRefType>(info.buffer.getType());
  return VectorType::get(shape, memTy.getElementType());
}
...
public:
  ...
  LogicalResult matchAndRewrite(FromOpTy op, OpAdaptor adaptor,
    ...
    auto copyToThreadOwnedVector = [&](Value memref, LowerInfo &info) -> Value {
      ...
      if (auto zeroOp = original.getDefiningOp<frisk::ZeroOp>()) {
        auto zeroVec =
            rewriter
                ...
                .getResult();
        s_buffer_replace[original] = zeroVec;
        return zeroVec;
      }
    ...
    s_buffer_replace[op.getResult()] = newOp.getResult();
  ...
  LogicalResult matchAndRewrite(frisk::ReduceOp op, OpAdaptor adaptor,
    ...
    if (dstLoopShape.empty()) {
      return op.emitOpError("rank-0 reduce destination is not supported");
	    ...
	    Value srcVector = getVectorReplacement(op.getSrc(), op.getOperation());
...
public:
  ...
  LogicalResult matchAndRewrite(frisk::ConvertLayoutOp op, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const override {
    ...
    if (getVectorReplacement(memref, op.getOperation())) {
      rewriter.eraseOp(op);
