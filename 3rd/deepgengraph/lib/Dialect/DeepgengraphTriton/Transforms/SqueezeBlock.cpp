...
#define GEN_PASS_DEF_DEEPGENGRAPHTRITONSQUEEZEBLOCK
    ...
    while (!q.empty()) {
      ...
      for (auto &use : val.getUses()) {
        ...
        if (auto scf_for_op = dyn_cast<scf::ForOp>(user)) {
          ...
          auto new_val = rewriter.create<::mlir::deepgengraph::UnsqueezeOp>(user->getLoc(), use.get(), target_dim);
...
class SqueezeBlockPass : public ::mlir::deepgengraph::triton::impl::DeepgengraphTritonSqueezeBlockBase<SqueezeBlockPass> {
public:
